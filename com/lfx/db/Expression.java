...
public class Expression {
   ...
   private static final byte  // values for code array; values >= 0 are indices into constants array
       ...
       INT = -21, UNARYMINUS = -22, VARIABLE = -23, VARIABLEY=-24, VARIABLEZ=-25;       
       
   private static String[] functionNames =  // names of standard functions, used during parsing
       { "sin", "cos", "tan", "cot", "sec",
         "csc", "arcsin", "arccos", "arctan", "exp",
         "ln", "log10", "log2", "abs", "sqrt", "int" };
         
         
   private double eval(double variable, double variabley, double variablez) { // evaluate this expression for this value of the variable
      try {
         int top = 0;
   ...
   private int computeStackUsage() {  // call after code[] is computed
      ...
      for (int i = 0; i < codeSize; i++) {
         if (code[i] >= 0 || code[i] == VARIABLE || code[i] == VARIABLEY || code[i] == VARIABLEZ ) {
            s++;
            if (s > max)
               max = s;
         }
