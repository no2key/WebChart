...
/*
    ...
    can involve:
     ...
     June 2002: Changed deprecated method "Character.isSpace" to
                "Character.isWhitespace".
*/

package com.lfx.db;

public class DBRowCacheExpression {

   //----------------- public interface ---------------------------------------

   public DBRowCacheExpression(String definition) {
   ...
   public double value(DBRowCache data, int row)
   {
          return eval(data,row);
   }

   public double value(DBRowCache data, int row, int grpcols[])
   {
          return eval(data,row, grpcols);
   }

   public double value(VariableTable vt)
   {
          return eval(vt);
   }
   ...
   private String[] varlist;   // Contain the variable name for specific code
   ...
   private static final byte  // values for code array; values >= 0 are indices into constants array
       PLUS = -1,   MINUS = -2,   TIMES = -3,   DIVIDE = -4, MOD=-5, POWER = -6, 
       SIN = -7,    COS = -8,     TAN = -9,     COT = -10,     SEC = -11,
       CSC = -12,   ARCSIN = -13, ARCCOS = -14, ARCTAN = -15, EXP = -16,    
       LN = -17,    LOG10 = -18,  LOG2 = -19,   ABS = -20,   SQRT = -21,
       INT = -22,   SIGN=-23, UNARYMINUS = -24, VARIABLEANY=-25;       
       
   private static String[] functionNames =  // names of standard functions, used during parsing
       { "sin", "cos", "tan", "cot", "sec",
         "csc", "arcsin", "arccos", "arctan", "exp",
         "ln", "log10", "log2", "abs", "sqrt", "int", "sign" };
         
         
   private double eval(DBRowCache data, int row) { // evaluate this expression for this value of the variable
      try {
         int top = 0;
         for (int i = 0; i < codeSize; i++) {
            ...
            else if (code[i] == VARIABLEANY) {
	       if (data != null)
	       {
	           stack[top++] = data.getExprValue(row, varlist[i]);
	       }
	       else
               {
   	           stack[top++] = 0.0f;
                ...
                switch (code[i]) {
                   ...
                   case SIGN:  ans = (x==0.0f ? 0f : (x>0 ? 1.0: -1.0));  break;
   ...
   private double eval(DBRowCache data, int row, int grpcols[]) { // evaluate this expression for this value of the variable
      try {
         int top = 0;
         for (int i = 0; i < codeSize; i++) {
            if (code[i] >= 0)
	       ...
	       if (data != null)
	       {
	           stack[top++] = data.getExprValue(row, varlist[i], grpcols);
	       }
	       else
               {
   	           stack[top++] = 0.0f;
   ...
   private double eval(VariableTable vt) { // evaluate this expression for this value of the variable
      try {
         int top = 0;
         for (int i = 0; i < codeSize; i++) {
            if (code[i] >= 0)
	       ...
	       if (vt != null && vt.getValue(varlist[i]) != null)
	       {
  	           Object itemval = vt.getValue(varlist[i]);
		   if (itemval instanceof Number)
		   {
			stack[top++] = ((Number)itemval).doubleValue();
		   }
		   else
		   {
		      try {
			stack[top++] = Double.valueOf(itemval.toString()).doubleValue();
		      }
		      catch (NumberFormatException nfe) {stack[top++] = 0.0f;}
		   }
	       }
	       else
               {
   ...
   private int computeStackUsage() {  // call after code[] is computed
      ...
      for (int i = 0; i < codeSize; i++) {
         if (code[i] >= 0 || code[i] == VARIABLEANY) {
            s++;
            if (s > max)
               max = s;
         }
   ...
   private void parse(String definition) {  // Parse the definition and produce all
      ...
      varlist = new String[definition.length()];
   ...
   private void parseTerm() {
      ...
      while (next() == '*' || next() == '/' || next() == '%') {
         ...
         pos++;
         parseFactor();
         code[codeSize++] = (op == '*')? TIMES : ((op == '/')? DIVIDE : MOD);
         skip();
      }
   }
   ...
   private void parsePrimary() {
      ...
      if (ch == '$') {
         pos++;
	 varlist[codeSize]   = parseVariable();
         code[codeSize++] = VARIABLEANY;
      }
