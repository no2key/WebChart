...
public class WebChartServlet extends BaseServlet {
	...
	if (reqfile.endsWith(concatString("/showimage",file_ext)))
	{
		...
		if (vt.exists("ID"))	
		{
			...
			byte imagedata[] = ImageCache.getImageContent(vt.getString("ID"));
			if (imagedata != null)
			{
				out.write(imagedata);
				out.flush();	
			}
			if (!"YES".equalsIgnoreCase(vt.getString("WEBCHART.KEEP_CACHE_IMAGE")) 
			      && "YES".equalsIgnoreCase(vt.getString("DEL")))
				ImageCache.removeContent(vt.getString("ID"));
		}
		return;
	}
	         ...
	         reqfile.endsWith(concatString("/sysdbmgr",file_ext)) )
	{
	        ...
	        else if (reqfile.endsWith("/syspassword"+file_ext))
		{
			...
			forward(request,response,vt.parseURLString(vt.getString("ERROR_PAGE")));
			return;
		}
	...
	vt.setValue("REQUEST.URL",request.getServletPath());
	...
	if (!checkAccess(login_id,login_role,vt))
	{
		if (vt.getString("WEBCHART.ACCESSDENY") != null)
		    forward(response,vt.parseURLString(vt.getString("WEBCHART.ACCESSDENY")));
		else 
		    forward(response,"sysloginform"+file_ext+"?login_page="+vt.getString("REQUEST.FILE"));
		return;
	}
	...
	if (imageonly.equals("YES"))
	{
		...
		if (cache_content == null)
		{
			...
			if ("JFREE".equalsIgnoreCase(vt.getString("WEBCHART.ENGINE")))
        ...
        else if (xsldoc != null && xsldoc.trim().length()>0 && xsl_file.exists() && "HTML".equals(document_type))
	{
	        java.io.OutputStream out = response.getOutputStream();
		setHTMLContentType(response,vt);
		response.setHeader("Cache-Control", "no-cache"); 
